...
class AppointmentController extends Controller
{
    ...
    public function getAppointmentById(Request $request, $id)
    {
        try {
            $panditjiId = $request->id;
            $apt = new appointmentDetails();
            $appointmentDetail = $apt->getAppointmentDetailsByID($panditjiId, $id);
            // print_r($appointmentDetail);
            if ($appointmentDetail == false) {
                return response()->json(['status' => false, 'message' => 'No Pooja Created', 'data' => []], 200);
            } else {
    ...
    /**
     * @OA\Delete(
     *      path="/api/v1/pandit/appointment/delete/{id}",
     ...
     *             @OA\Property(property="status", type="boolean", example="true"),
     *             @OA\Property(property="code",type="integer", example="200"),
     *             @OA\Property(property="Appointment deleted successfully")
     *          )
     *       ),
     *     @OA\Response(
     *          response=500, description="Internal Server Error",
    ...
    public function deleteAppointment(Request $request, $id)
    {
        try {
            ...
            $panditjiId = $request->id;
            $apt = new appointmentDetails();
            $appointmentExistOrNot = $apt->IsAppointment($panditjiId, $id);

            if (!$appointmentExistOrNot) {
                return response()->json(['status' => false, 'message' => 'Resource not found'], 200);
            }

            $appointmentExistOrNot->delete();
    ...
    /**
     * @OA\Put(
     *      path="/api/v1/pandit/appointment/updateAppointment/{id}",
     *      operationId="UpdateAppointment",
     *      tags={"Appointment"},
     *      summary="Update the appointment for the yajman",
     *      @OA\Parameter(
     *          name="Authorization",
     *          in="header",
     *          required=true,
     ...
     *             @OA\Property(property="status", type="boolean", example="false"),
     *             @OA\Property(property="code",type="integer", example="403"),
     *             @OA\Property(property="message", type="string", example="Appointmnet updation Failed")
     *          )
     *       ),
     *     @OA\Response(
     *          response=500, description="Internal Server Error",
    ...
    public function updateAppointment(Request $request,$id){
        try {
            ...
            $panditji = new appointmentDetails();
            $appointmentCreatedOrNot = $panditji->getAppointmentDetailsByID($panditjiId,$id);

            if ($appointmentCreatedOrNot == false) {
                return response()->json(['status' => false, 'message' => 'Appointment does not exist'], 400);
            } else {
                $apnt = new appointmentDetails();
                $apnt->yajman_name = $input['yajman_name'];
                $apnt->yajman_mobile_no = $input['mobile_number'];
                ...
                if ($save) {
                    return response()->json(['status' => true, 'message' => 'Appointment updated successfully', 'data' =>$appointmentCreatedOrNot], 200);
                } else {
                    return response()->json(['status' => false, 'error' => 'Something went wrong', 'message' => "Appointement updatation Failed"], 500);
                }
            }
        } catch (\Throwable $th) {
